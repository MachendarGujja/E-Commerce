...
const Product = ({product}) => {
  // const handleClose = () => setOpen(false);
    const {
        ...
        dispatchMain,dispatchFilter} = CartData();

    // console.log(product);
  return (
    <li className='cards' >
        <img onClick={()=>dispatchFilter({type:'OPEN',payload:product})} style={{width:'100%',cursor:"pointer",height:138,borderTopLeftRadius:4,borderTopRightRadius:4}} src={product.image} alt={product.title}/>
        <div className='bot-css'>
          <div style={{marginTop:10,cursor:"pointer"}} onClick={()=>dispatchFilter({type:'OPEN',payload:product})}>
        <h5  className="intro">{product.name.length>35?`${product.title.substring(0,17)}...`:`${product.name}`}</h5>
          </div>
